  ...
  if (seg[0] === 'gameplay') {
    ...
    if (seg[1] === 'enemy' && seg[2]) {
      ...
      if (req.method === 'POST') {
        ...
        const assetExists = await fs.stat(path.join(RANGERS_DIR, data.assetVariantId)).then(s => s.isDirectory()).catch(() => false)
        if (!assetExists) return sendJson(res, 400, { error: 'unknown assetVariantId' })
      ...
      if (req.method === 'DELETE') {
        ...
        if (stages.some(stage => stage.waves?.some(w => w.enemies?.some(row => row.enemyId === id)))) {
          return sendJson(res, 409, { error: 'enemy is used by a stage' })
        }
        const exists = await fs.stat(file).then(s => s.isFile()).catch(() => false)
        if (!exists) return sendJson(res, 404, { error: 'not found' })
        await fs.unlink(file)
    ...
    if (seg[1] === 'stage' && seg[2]) {
      ...
      if (req.method === 'POST') {
        ...
        const enemyIds = new Set((await listGameplayDocs(ENEMIES_DIR)).map(row => row.id))
        for (const wave of data?.waves ?? []) for (const row of wave?.enemies ?? []) {
          if (!enemyIds.has(row.enemyId)) errors.push('unknown enemyId: ' + row.enemyId)
        }

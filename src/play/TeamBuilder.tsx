...
export default function TeamBuilder({ data, formation, setFormation, onStart, busy, message }: {
  ...
  const canStart = formation.every(team => SLOT_KEYS.some(k => rowOf(team[k])))
  ...
  return <main className="ep-team-page">
    <div className="ep-page-heading"><span className="ep-eyebrow">TEAM / PRACTICE</span><h1>編組隊伍</h1><p>點選空位，依序選擇角色與職業。每隊最多 5 位角色，同一角色每隊限出場一次。</p></div>
    <div className="ep-team-boards">{([0, 1] as const).map(side => <section className={'ep-team-board side-' + side} key={side}>
    ...
    <footer className="ep-team-footer"><p>{canStart ? '隊伍已準備完成。' : '請替雙方各選擇至少 1 位角色。'}</p><button className="ep-primary" disabled={busy || !canStart} onClick={onStart}>{busy ? '載入中…' : '開始對戰 →'}</button></footer>
  </main>
}
